...
```{r - rps comparison diff, echo = FALSE, message = FALSE, fig.width = 10, fig.height = 5, fig.show='hold', fig.align='center'}
...
* Power Odds
    ...
    + Most accurate method for $\text{Original Probability} < \approx 0.25$ & $\approx0.5 < \text{Original Probability}$.
    + Least accurate method for $\approx 0.3 < \text{Original Probability} < \approx 0.4$.
* Proportional Odds
    + More accurate than the 'Original Odds' for $\approx 0.2 < \text{Original Probability} < \approx 0.65$.
    + Most accurate method for $\approx 0.3 < \text{Original Probability} < \approx 0.4$.
    + Least accurate method for $\text{Original Probability} < \approx 0.3$ & $\approx 0.4 < \text{Original Probability}$
* Ratio Odds
    + More accurate than the 'Original Odds' for $\approx 0.1 < \text{Original Probability} < \approx 0.8$.
    + It appears there are two very small regions around $\text{Original Probability} \approx 0.3$ & $\text{Original Probability} \approx 0.4 where this method is the most accurate method.
...
```{r - rps comparison crossover points, echo = FALSE, message = FALSE, fig.width = 10, fig.height = 5, fig.show='hold', fig.align='center'}
...
# add geom area
plot_rps_test <- rps_comparison_cross_over_points %>% 
  ggplot(aes(original_probs, min_odds, colour = min_method)) + 
...
# add colour ribbons
# plot_rps_min <- plot_rps_min + 
#   geom_ribbon(aes(x=original_probs, ymin=-1, ymax=min_odds), fill="red", alpha=0.1, data = rps_comparison_cross_over_points %>% filter(min_method == "power_odds")) +
#   geom_ribbon(aes(x=original_probs, ymin=-1, ymax=min_odds), fill="blue", alpha=0.1, data = rps_comparison_cross_over_points %>% filter(min_method == "ratio_odds")) + 
#   geom_ribbon(aes(x=original_probs, ymin=-1, ymax=min_odds), fill="green", alpha=0.1, data = rps_comparison_cross_over_points %>% filter(min_method == "proportional_odds"))



grid.arrange(plot_rps_diff, nullGrob(), plot_rps_min,  top = textGrob("A Comparison of the Different Odds Methods"), widths=c(0.475, 0.05, 0.475))
