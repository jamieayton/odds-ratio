...
plot_probs <- plot_data %>% 
  ggplot(aes(x = 1/original_odds, y = 1/odds_value, color = odds_method)) + 
  geom_smooth() + 
  geom_abline(slope = 1, intercept = 0, lty = 2) + 
...
# probs plot
plot_probs <- plot_data %>% 
  ggplot(aes(1/original_odds, probs_scalar_multiple, color = odds_method)) + 
  geom_smooth() + 
  geom_hline(yintercept = 1, lty = 2) + 
...
Lets define the RPS Scalar $\lambda$ to be such that $\text{RPS}_{\text{True Odds}} = \lambda * \text{RPS}_{\text{Original Odds}}$. Then if $\lambda < 1$ this means that the 'True Odds' produced are more accurate than the 'Original odds', and conversely if $\lambda > 1$ then the 'Original odds' are more accurate.  

  

```{r plot - rps comparison, echo = FALSE, message = FALSE, fig.width = 10, fig.height = 5, fig.show='hold', fig.align='center'}
...
plot_data_rps <- plot_data_rps %>% 
  ...
  mutate(
    rps_scalar = rps/original_rps, 
    rps_diff = rps - original_rps
  )
  

# plot 'original_odds' vs 'rps scalar'
plot_rps_diff <- plot_data_rps %>% 
  ggplot(aes(1/original_odds, rps_scalar, colour = odds_method)) + 
  geom_smooth(se = TRUE) + 
  geom_hline(yintercept = 1, lty = 2) + 
    coord_cartesian(xlim = c(0, 1), ylim = c(0.975, 1.075)) + 
  scale_colour_discrete(
    name = "Odds Method",
    breaks = c("power_odds", "proportional_odds", "ratio_odds"),
    labels = c("Power Odds", "Proportional Odds", "Ratio Odds")
  ...
  theme_classic() + 
  theme(legend.justification = c(0.45, 0.8), legend.position = c(0.45, 0.8)) + 
  labs(x = "Original Probabilities \n Fig 1.5", y = "RPS Scalar", title = "Comparing the Accuracy of Various Methods for Determining True Odds")
...
Lets break down the graph (Fig 1.5) into three approximate sections on the 'Original Probabilities' axis:  

* $0 < \text{Original Probability} < 0.3$  
    + The 'Power Odds' method is the most accurate method for this range of Probabilities, since the RPS Scalar is below 1 & the RPS scalar value is the lowest for all the odds method.
    + The 'Ratio Odds' & the 'Proportional Odds' methods are less accurate than the original odds values since the RPS Scalar are above the dotted line at RPS Scalar = 1.
* $0.3 < \text{Original Probability} < 0.5$  
    + The 'Power Odds' method is the least accurate method for this range of Probabilities, and is less accurate than the original odds values.
    + The 'Ratio Odds' & the 'Proportional Odds' methods are more accurate than the 'Original Odds'.
    + The 'Proportional Odds' method is the most accurate method in this Probability range.
* $0.5 < \text{Original Probability} < 1$  
    + Same pattern as the region Original Probability < 0.3. 
...
What determines these cut off points where the most accurate method switches between the various methods?  
...
2. Overround - as the overround increase does this effect the cut off points or promote a different method?
3. Bookmaker - do we see the same patterns for all bookmakers?
...
```{r - rps crossover points, echo = FALSE, message = FALSE}

test <- tibble(original_probs = seq(0, 1, by=0.0001)) %>% 
  mutate(
...
rps_plot <- tibble(original_probs = seq(0, 1, by=0.0001)) %>% 
  mutate(
    ratio_odds = as.vector(predict(loess_odds_methods$fit[loess_odds_methods$odds_method == "ratio_odds"][[1]], newdata = .)), 
    power_odds = as.vector(predict(loess_odds_methods$fit[loess_odds_methods$odds_method == "power_odds"][[1]], newdata = .)), 
    proportional_odds = as.vector(predict(loess_odds_methods$fit[loess_odds_methods$odds_method == "proportional_odds"][[1]], newdata = .))
  ...
  drop_na() %>% 
  gather(odds_method, rps_diff, which(colnames(.) %in% c("ratio_odds", "power_odds", "proportional_odds"))) %>% 
  ggplot(aes(1/original_probs, rps_diff, colour = odds_method)) + 
  geom_line() + 
  geom_hline(yintercept = 0, lty = 2) + 
  coord_cartesian(xlim = c(1, 10), ylim = c(-0.0005, 0.0005)) + 
  scale_colour_discrete(
    name = "Odds Method",
    breaks = c("power_odds", "proportional_odds", "ratio_odds"),
    labels = c("Power Odds", "Proportional Odds", "Ratio Odds")
