...
As an example - lets take the RPS Comparison from above where we estimated cross overpoint at Original Probability values of 0.3 & 0.5. 




```{r - rps comparison crossover points, echo = FALSE, message = FALSE}
...
# original plot
rps_comparison_cross_over_points %>% 
  gather(odds_method, rps_diff, which(colnames(.) %in% c("ratio_odds", "power_odds", "proportional_odds"))) %>% 
  ggplot(aes(original_probs, rps_diff, colour = odds_method)) + 
  geom_smooth(se = TRUE) + 
  coord_cartesian(xlim = c(0, 1), ylim = c(-0.00025, 0.00025)) + 
  geom_hline(yintercept = 0, lty = 2) + 
  scale_colour_discrete(
    name = "Odds Method",
    breaks = c("power_odds", "proportional_odds", "ratio_odds"),
